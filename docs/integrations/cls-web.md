...
CLS 的 `StockID` 在 Provider 边界规范化：已验证的 `sh600000`、`sz000001`、
`bj920118` 前缀格式保持支持，真实上游出现的北交所 `920403.BJ` 格式也映射为同一个
规范北京市场身份并在映射后去重。该兼容只覆盖精确大写 `.BJ`；尚未从该合同独立验证的
`.SH`、`.SZ`、小写或其他后缀仍明确返回 `Protocol`。客户端不需要、也不应解析或改写
Provider 原始证券身份。

深圳关联证券按深交所公开代码区间判定：`300000..=309799` 是创业板股票，因此真实上游
`sz302132` 保留为 Shenzhen Equity；`309800..=309999` 是创业板存托凭证，而 Core 尚无
对应资产类别，仍明确拒绝。不得再用仅列举 `300`/`301` 前缀的历史实现阻断后续合法
创业板号段，也不得把存托凭证伪装成股票。

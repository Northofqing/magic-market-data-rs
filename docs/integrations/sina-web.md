...
2026-08-27 的真实页面在官方公告 URL 查询串中使用未转义的 `&id=` 分隔符。HTML
解码器现在只把带分号的已知/数字实体当作实体；普通查询分隔符原样保留并继续通过后续
Sina host、端口、凭据和 URL 结构校验。`&amp` 等已知但缺少分号的实体以及未知的
`&name;` 仍明确拒绝，不能借此绕过 URL 合同。
...
同一探针与 `page-pinned.ps1` 在 2026-09-23 测得来源的置顶行：`sz002131` 第 1 页首行
带 `[置顶]` 前缀、时间为 `2026-09-23 00:52`（href 落在 `wq.finance.sina.com.cn`），
却排在 `16:39` 起的新闻行之前，使该页出现一处逆序；第 2 至 5 页无逆序，同一时刻抽查
的 `sh600519`、`sh600396`、`sz000001`、`sh600667`、`sh600703`、`sz002421` 均无置顶
行。置顶行仍按 BR-025 逐项校验（身份、MIME、发布时间与未来时间），校验后不进入记录
集、不参与本页逆序校验，也不参与本页时间极值。

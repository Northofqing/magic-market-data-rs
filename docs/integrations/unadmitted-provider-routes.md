...
| `Jin10 / EconomicCalendar` | The public flash interface still exposes bounded release-related flashes, but Jin10 ended its free calendar/API embedding service on 2025-12-01. | Keep diagnostic: a latest-flash window cannot prove a complete calendar. | `EconomicReleaseObservations / Jin10` exposes only already-published structured rows in the current rolling window. It is not a calendar substitute. Other admitted economic datasets also retain their own Provider identity. |

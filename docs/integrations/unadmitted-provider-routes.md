...
This matrix records the eleven repository-unadmitted Provider-by-operation
...
| `EmQuant / RealtimeQuotes` | Official `css` is already implemented for the quote indicator set. The current account returned `10001012 / EQERR_ACCESS_INSUFFICIENCE`. | Keep diagnostic; entitlement failure returns no production records. | `Tencent`, `Tdx`, `Sina`, `Szse`, or observation-time `HithinkFinance`. |

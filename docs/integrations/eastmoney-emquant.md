...
本机在 2026-07-23 审核通过后再次完成短信激活，`userInfo` 的修改时间与设备登录
日志均已刷新；关闭激活器后的干净 SDK 进程仍返回
`10001003 (EQERR_NO_ACCESS)`。另用最小官方 ABI 调用执行
`start(nullptr, "ForceLogin=1")`，结果仍为同一码。官方随包头文件与 PDF 将其定义
为“用户无 API 权限”，因此当前剩余阻塞是 EMQuant 登录服务尚未给账号下发 API
产品 entitlement，不是令牌、路径、签名、网络、并发会话或桥接代码。需要由
Choice/QuantAPI 后台确认“EMQuant API 产品权限”已经生效，而不只是设备激活审核
通过；权限生效后可直接重跑探针。项目已经为无权限、权限过期、Level-2 无权限、
登录数上限、设备不一致和令牌过期提供可操作诊断。

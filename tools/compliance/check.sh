...
required=(AGENTS.md Cargo.toml Cargo.lock LICENSE-APACHE LICENSE-MIT LICENSES/tdxrs-MIT.txt docs/ENGINEERING_RULES.md docs/business_rules.md docs/DEPLOYMENT.md docs/MULTI_PROVIDER_ROUTING.md docs/integrations/tencent-web.md docs/integrations/sina-web.md crates/magic-market-router/Cargo.toml crates/magic-tencent-rs/Cargo.toml crates/magic-sina-rs/Cargo.toml)
for path in "${required[@]}"; do test -s "$path" || { echo "missing required file: $path" >&2; exit 1; }; done
rg -q '^members = \["crates/magic-market-core", "crates/magic-market-router", "crates/magic-tdx-rs", "crates/magic-emquant-rs", "crates/magic-tencent-rs", "crates/magic-sina-rs"\]$' Cargo.toml

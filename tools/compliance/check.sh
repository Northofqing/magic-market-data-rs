...
required=(AGENTS.md Cargo.toml Cargo.lock LICENSE-APACHE LICENSE-MIT LICENSES/tdxrs-MIT.txt docs/ENGINEERING_RULES.md docs/business_rules.md docs/DEPLOYMENT.md docs/integrations/tencent-web.md crates/magic-tencent-rs/Cargo.toml)
for path in "${required[@]}"; do test -s "$path" || { echo "missing required file: $path" >&2; exit 1; }; done
rg -q '^members = \["crates/magic-market-core", "crates/magic-tdx-rs", "crates/magic-emquant-rs", "crates/magic-tencent-rs"\]$' Cargo.toml

...
- 在 Windows 自动发现当前用户会话中的通达信客户端，通过固定本机 TQ-Local 只读接口
  获取价格、累计成交量和累计成交额。
- Sina 集合竞价未成交时的 `0.000` 现价不会否决同包五档；竞价挂单量能使用现有
  `OrderBooks(provider=Sina)`，不伪造成交价，也不把五档冒充完整 Level-2 `Auctions`。
...
- 提供同花顺扶摇 `live`/`final` 当前竞价观测，保留单一有符号未匹配量和响应观察时间，
  不伪造交易日、源时间或买卖方向。
...
保持 `null`，不会从普通行情推导。同花顺扶摇另有正式
`CurrentAuctionObservations` 窄合同：调用方明确选择 `live` 或 `final`，记录保留源直接给出的
成交价、昨收、成交量（严格从手换算为股）、成交额、单一有符号未匹配量及其余竞价指标；

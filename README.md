...
- 提供价格、成交额、成交量的确定性异动计算和事件合同；当前异动结果仍属于影子数据，
  等交易时段证据和生产阈值确认后再准入。
- 提供指数行情、分时形态、结果日线和涨停池复盘等组合数据产品；每项能力独立准入。
...
不加载厂商 DLL，不依赖 Python，也不调用账户或交易接口。当前生产准入仅覆盖
`Now`、`Volume` 和 `Amount`；源时间、`source_record_count`、本地 OHLC/昨收和异动
结果仍保持未准入。
...
- 缺失值不填零，本地时间不冒充 Provider 源时间，诊断成功不提升生产准入。

...
fn constructors_helpers_limits_and_transport_failures_are_explicit() {
    assert!(XinhuaClient::new().is_ok());
    assert!(XinhuaClient::with_timeout(Duration::ZERO).is_err());
    ...
    assert!(matches!(tracker_error("x"), XinhuaError::Transport(_)));
    let client = XinhuaClient::with_transport(Failure).unwrap();
    assert!(format!("{client:?}").contains("XinhuaClient"));
    assert!(client

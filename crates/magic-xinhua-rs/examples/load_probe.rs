...
fn run_probe() -> Result<ProbeStatus, Box<dyn Error>> {
    ...
    let policy =
        ProbeAdmissionPolicy::new(ProviderId::XinhuaFinance).with_max_source_age(MAX_SOURCE_AGE)?;
    let mut records = 0;
    ...
    println!(
        "provider=xinhua-finance requests={} records={} actual_request_starts={} actual_minimum_start_gap_ms={} observed_maximum_concurrency={}",
        REQUESTS,
        records,
        snapshot.request_starts(),
        snapshot
            .minimum_start_gap()
            .ok_or("multiple Xinhua requests did not record a start gap")?
            .as_millis(),

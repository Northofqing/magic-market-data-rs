...
use magic_xinhua_rs::{XinhuaClient, XinhuaError, GLOBAL_NEWS_ADMITTED};
...
fn unsupported_instrument_news_performs_no_io() {
    ...
    let client =
        XinhuaClient::with_transport(CountingTransport(calls.clone())).expect("test client");
...
impl HttpTransport for RecordingTransport {
    fn execute(&self, request: &HttpRequest) -> Result<HttpResponse, TransportError> {
        ...
        Ok(HttpResponse::new(
            ...
            include_bytes!("fixtures/news.html").to_vec(),
        ))
    }
}
...
fn probe_uses_the_exact_bounded_first_page_request_and_tracks_it() {
    ...
    let client =
        XinhuaClient::with_transport(RecordingTransport(requests.clone())).expect("test client");
    let batch = client
    ...
    assert_eq!(requests[0].url(), "https://www.cnfin.com/news/index.html");
    assert_eq!(
        requests[0].headers(),
        [

...
#[test]
fn discovery_identity_and_range_helpers_cover_every_verified_boundary() {
    for (code, exchange) in [
        ("600396", Exchange::Shanghai),
        ("000001", Exchange::Shenzhen),
        ("300001", Exchange::Shenzhen),
        ("430001", Exchange::Beijing),
        ("830001", Exchange::Beijing),
        ("920001", Exchange::Beijing),
    ] {
        assert_eq!(discovered_equity(code).unwrap().exchange(), exchange);
    }
    for code in ["", "60039", "60039A"] {
        assert!(matches!(
            discovered_equity(code),
            Err(CninfoError::Schema(_))
        ));
    }
    for code in ["100001", "500001", "700001", "900001"] {
        assert!(matches!(
            discovered_equity(code),
            Err(CninfoError::Unsupported(_))
        ));
    ...
    .unwrap();
    assert!(ensure_discovery_range("bad", &request).is_err());
    assert!(ensure_discovery_range("2026-07-22T00:00:00+08:00", &request).is_err());
    assert!(ensure_discovery_range("2026-07-25T00:00:00+08:00", &request).is_err());
    assert!(ensure_discovery_range("2026-07-23T00:00:00+08:00", &request).is_ok());
    assert!(ensure_discovery_range("2026-07-24T23:59:59+08:00", &request).is_ok());
}

#[test]
fn discovery_pagination_rejects_every_incomplete_or_contradictory_shape() {
    let request = AnnouncementDiscoveryRequest::new(
        magic_market_core::IsoDate::new("2026-07-24").unwrap(),
        magic_market_core::IsoDate::new("2026-07-24").unwrap(),
        magic_market_core::PositiveU32::new(10).unwrap(),
    ...
    let valid_row = serde_json::json!({
        ...
        "adjunctUrl": "finalpage/2026-07-24/A-1.PDF"
    });
    let cases = [
        serde_json::json!({
            "totalRecordNum": 1,
    ...
    for value in cases {
        ...
        assert!(client.discover_announcements(&request).is_err(), "{value}");
    }
}

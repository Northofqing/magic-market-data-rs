...
fn run_probe() -> Result<ProbeStatus, Box<dyn Error>> {
    ...
    println!("provider=yicai capabilities={capabilities:?}");
    ...
    {
        return Err("Yicai metadata probe exposed summary or content".into());
    }
    println!(
    ...
    for item in batch.records() {
        println!(
            "provider=Yicai id={} title={} publisher={} url={} published_at={} source_at={:?} observed_at={} batch_id={}",
            item.item_id,
            item.title,
            item.publisher,
            item.canonical_url,

...
// 服务器列表 (三层结构)
//
// 2026-05-03 全量探测 (101台 → 33台可达); 2026-08-05 复测剔除 count=800 空数据/无响应服务器 (19台可达);
// 2026-09-23 复测 (87台遍历, 18台可达) 重排优先级:
//   PRIMARY_SERVERS (7台): 实测 K 线可用 + 连接池默认使用
...
/// 优先组 (7台) — 连接池 connect_to_any() 默认遍历
/// 选取原则: K线数据可用 + 响应稳定 (跨 IP 段/运营商的余量见 2026-09-23 说明)
///
/// 2026-07-08 修正:
///   - 移除华林4 (202.96.138.90) — 连接不稳定，频繁超时
///   - 移除海通8/海通3/海通2/海通4 — K线接口返回空数据 (count API 正常)
///   - 补入: 国信1/华林7/杭州电信J2/杭州电信J1/杭州电信J4
///
/// 2026-07-17 修正:
///   - 移除广发1 (119.29.19.242) — K线/行情接口返回空数据 (count/finance/xdxr 正常)
///   - 补入: 杭州电信J4 (115.238.90.165)
///
/// 2026-08-05 修正 (原始 K 线探针逐台实测):
///   - 移除广发13 (183.60.224.177) — K线接口返回 count=800 空数据 (E2103)
///   - 移除国信1 (182.131.3.252) — 连接无响应 (多轮探测超时)
///   - 补入: 华林6 (218.106.92.183) / 安信15 (59.36.5.11) — 实测返回完整 K 线
///
/// 2026-09-23 修正 (87 台逐一实测: 600519/000001/920118 × 日线/RI_K × count=5/40/800):
///   - **上一版十台全部**回 count=800 空 payload (body 恰好 2 字节 `20 03`), 即 E2103。
///     其中华林6/安信15 正是 2026-08-05 因"返回完整 K 线"补入的 —— 说明**这个行为会漂移**,
///     静态名单必然再次过期。所以本次同时补了客户端侧的处理:
///     撞上这类服务器就拉黑换台, 而不是把 E2103 抛给调用方
///     (见 `net::utils::declares_rows_without_payload`)。
///   - 补入: 国泰君安 8/9/10/11/12/13/14 —— 上述六种形状全部返回完整数据。
///   - 已知代价: 这七台同在 117.34.114.0/24, 本轮没有实测可用的第二网段, 因此优先组暂时
///     没有跨运营商冗余。旧的十台留在 ALL_KNOWN_SERVERS 里兜底, 恢复供数后可再提回。
...
pub const ALL_KNOWN_SERVERS: &[(&str, &str, u16)] = &[
    // ── 已探测可达 (2026-09-23 实测 18 台, 含 PRIMARY 7 台) ──
    // 2026-09-23 逐台实测分类:
    //   K线正常 (7台, 已提为 PRIMARY): 国泰君安 8/9/10/11/12/13/14
    //   K线回 count=800 空 payload / E2103 (11台): 华林8、杭州联通J2、华林5、杭州电信J3、
    //     上海电信Z1、华林7、杭州电信J2、杭州电信J1、杭州电信J4、华林6、安信15
    //     —— 保留为兜底候选; 客户端撞上它们会拉黑并换台, 恢复供数后自动重新可用
    //   不可达 (3台): 国泰君安7 (117.34.114.13) / 15 (117.34.114.30) / 16 (117.34.114.31)
    // 2026-08-05 原始探针逐台实测剔除:
    //   海通1/2/3/4/5/8/9、广发1/13/14、上证云成都/北京 — K线返回 count=800 空数据 (E2103)
    //   国信1 — 连接无响应; 华林4 — 连接超时

...
fn fixture_manifest_is_complete() {
    ...
    let manifest: Vec<Value> = serde_json::from_str(&fs::read_to_string(format!("{root}manifest.json")).unwrap()).unwrap();
    assert!(!manifest.is_empty());
    for entry in manifest {
        let path = entry.get("path").and_then(Value::as_str).expect("fixture path");
        let operation = entry.get("operation").and_then(Value::as_str).expect("fixture operation");

    ...
    match client.connect_to_any(Some(3.0)) {
        Ok(true) => {
            ...
            match inner.get_index_bars(4, 1, "000001", 0, 5, 0) {
                Ok(items) => println!("index_kline count={} first_datetime={}", items.len(), items.first().map_or("none", |v| v.datetime.as_str())),
            ...
            match inner.get_history_minute_time_data(1, "600519", 20260721) { Ok(items) => println!("minute_history date=20260721 count={}", items.len()), Err(error) => println!("minute_history=error error={error}") }
            ...
            match inner.get_history_transaction_data(1, "600519", 0, 20, 20260721) { Ok(items) => println!("transactions_history date=20260721 count={}", items.len()), Err(error) => println!("transactions_history=error error={error}") }
            ...
            match blocks.get_industry_blocks() { Ok(items) => println!("blocks_industry={}", items.len()), Err(error) => println!("blocks_industry=error error={error}") }
            match blocks.get_concept_blocks() { Ok(items) => println!("blocks_concept={}", items.len()), Err(error) => println!("blocks_concept=error error={error}") }
            match blocks.get_index_blocks() { Ok(items) => println!("blocks_index={}", items.len()), Err(error) => println!("blocks_index=error error={error}") }
        }

...
fn validate_load(requests: usize) -> Result<(), String> {
    if requests == 0 || requests > MAX_REQUESTS {
        return Err(format!(
            "MAGIC_THEPAPER_LOAD_REQUESTS must be between 1 and {MAX_REQUESTS}"
        ));
    }
    Ok(())
}
...
fn main() -> Result<(), Box<dyn Error>> {
    let requests = std::env::var("MAGIC_THEPAPER_LOAD_REQUESTS")
        .ok()
    ...
    println!(
        "provider=thepaper-finance-ssr-v1 requests={requests} concurrency=1 min_interval_ms={} successes={successes} failures={failures} records={records} elapsed_seconds={:.3} requests_per_second={rps:.3} latency_us_p50={} latency_us_p95={} latency_us_p99={} latency_us_max={}",

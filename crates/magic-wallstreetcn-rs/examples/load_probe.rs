use magic_market_core::{PositiveU32, ProviderId};
...
fn validate_load(requests: usize) -> Result<(), String> {
    if (1..=MAX_REQUESTS).contains(&requests) {
        ...
        Err(format!(
            "MAGIC_WALLSTREETCN_LOAD_REQUESTS must be between 1 and {MAX_REQUESTS}"
        ))
    }
}

fn main() -> Result<(), Box<dyn Error>> {
    let requests = std::env::var("MAGIC_WALLSTREETCN_LOAD_REQUESTS")
        .ok()
    ...
    for index in 0..requests {
        ...
        let batch_id = batch
            .provenance()
            .batch_id()
            .ok_or("WallstreetCN batch is missing its provenance batch ID")?;
        if batch.provenance().source() != "wallstreetcn-rss-v1"
            || batch.records().iter().any(|item| {
                item.summary.is_some()
                    || item.content.is_some()
                    || item.evidence.provider() != ProviderId::WallstreetCn
                    || item.evidence.batch_id() != batch_id
            })
        {
            return Err("WallstreetCN load response violated metadata provenance bounds".into());
        }
    ...
    println!(
        "provider=wallstreetcn-rss-v1 requests={requests} concurrency=1 min_interval_ms={} records={records} elapsed_seconds={:.3}",

...
mod tests {
    ...
    use magic_market_core::{
        AssetClass, Exchange, FlowInterval, FlowScope, InstrumentId, RatioUnit,
    };
    ...
    fn null_data_bad_rows_and_nonzero_rc_fail() {
        assert!(
            parse_fund_flow(br#"{"rc":0,"data":null}"#, scope(), FlowInterval::Minute1).is_err()
        );
        assert!(
            parse_fund_flow(br#"{"rc":1,"data":null}"#, scope(), FlowInterval::Minute1).is_err()
        );

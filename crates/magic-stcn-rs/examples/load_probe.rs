...
fn run_probe() -> Result<ProbeStatus, Box<dyn Error>> {
    let client = StcnClient::new()?;
    let policy = ProbeAdmissionPolicy::new(ProviderId::SecuritiesTimes)
    ...
    println!(
        "provider=securities-times requests={} records={} actual_request_starts={} actual_minimum_start_gap_ms={} observed_maximum_concurrency={}",
        REQUESTS,
        records,
        snapshot.request_starts(),
        snapshot
            .minimum_start_gap()
            .ok_or("multiple Securities Times requests did not record a start gap")?
            .as_millis(),

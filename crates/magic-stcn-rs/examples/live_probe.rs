...
fn run_probe() -> Result<ProbeStatus, Box<dyn Error>> {
    ...
    {
        return Err("Securities Times metadata probe exposed summary or content".into());
    }
    println!(
    ...
    for item in batch.records() {
        println!(
            "provider=SecuritiesTimes id={} title={} publisher={} url={} published_at={} source_at={:?} observed_at={} batch_id={}",
            item.item_id,
            item.title,
            item.publisher,
            item.canonical_url,

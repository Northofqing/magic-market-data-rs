...
fn constructors_helpers_limits_and_transport_failures_are_explicit() {
    assert!(StcnClient::new().is_ok());
    assert!(StcnClient::with_timeout(Duration::ZERO).is_err());
    ...
    assert!(matches!(tracker_error("x"), StcnError::Transport(_)));
    let client = StcnClient::with_transport(Failure).unwrap();
    assert!(format!("{client:?}").contains("StcnClient"));
    assert!(client
